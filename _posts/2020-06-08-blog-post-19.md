...
For a general n-joint open chain and a task space with coordinates $q \i, \mathbf{R}^{m}$, where $m \leq n$, the manipulability ellipsoid corresponds to the end-effector velocities for joint rates $\theta^{'}$ satisfying $||\theta^{'}|| = 1$, a unit sphere in the n-dimensional joint-velocity space. Assuming $J$ is invertible, the unit joint-velocity condition can be written as-  
<center>$1 = \theta^{'T}\theta^{'}$</center>  
<enter>$ = (J^{-1}q^{'})^{T}(J^{-1}q^{'})$</center>  
