...
In the previous part we saw that at a kinematic singularity, the robot's end-effector loses the ability to translate or rotate in one or more diretions. However, kinematic singularity is a binary proposition, i.e- a particular configuration is singular or not. I t is reasonable to ask if a non-singular configuration is close to being singular. The answer is yes. One can determine the directions in which the end-effector's ability to move is diminished, and to what extent. The manipulability ellipsoid allows one to visualize geometrically the directions in which the end-effector moves with the least effort and with the most effort. Manipulability ellipsoid for a 2R planar arm is presented in the figure.  
<p align="center"><img src="/images/ellipsoid.PNG"></p>  

For a general n-joint open chain and a task space with coordinates $q \in \mathbf{R}^{m}$, where $m \leq n$, the manipulability ellipsoid corresponds to the end-effector velocities for joint rates $\theta^{'}$ satisfying $||\theta^{'}|| = 1$, a unit sphere in the n-dimensional joint-velocity space. Assuming $J$ is invertible, the unit joint-velocity condition can be written as-  
<center>$1 = \theta^{'T}\theta^{'}$</center>  
<center>$ = (J^{-1}q^{'})^{T}(J^{-1}q^{'})$</center>  
<center>$ = q^{'T}J^{-T}J^{-1}q^{'}$</center>  
<center>$ = q^{'T}(JJ^{T})^{-1}q^{'}$</center>  
<center>$ = q^{'T}A^{-1}q^{'}$</center>  

If $J$ is full rank, (of rank m), the matrix $A = JJ^{T} \in \mathbf{R}^{m \times m}$ is square symmetric, and positive definite, as is $A^{-1}$.  

__References__
====== 

[Modern Robotics: Mechanics, Planning and Control, Kevin M. Lynch, Frank C. Park](http://hades.mech.northwestern.edu/index.php/Modern_Robotics)
