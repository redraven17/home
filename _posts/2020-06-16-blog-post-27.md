...
The $A^{*}$ search algorithm efficiently finds a minimum-cost path on a graph when
...
the start node, and a set of edges $\epsilon$, the $A^{*}$ algorithm makes use of the following
data structures:  
...
zero then $A^{*}$ always explores from the OPEN node that has been reached
with minimum past cost. This variant is called Dijkstra’s algorithm, which
preceded $A^{*}$ historically. Dijkstra’s algorithm is also guaranteed to find
a minimum-cost path but on many problems it runs more slowly than
$A^{*}$ owing to the lack of a heuristic look-ahead function to help guide the
search.  
...
__Suboptimal $A^{*}$ search__- If the heuristic cost-to-go is overestimated by
multiplying the optimistic heuristic by a constant factor $\eta$ > 1, the $A^{*}$
...
will not be guaranteed to be optimal. One possibility is to run $A^{*}$
